...
public class Reportt {
    ...
    void reportdis(ActionEvent event) {
        if(examcode.getSelectionModel().isEmpty()==false)
        {
            ...
            int[] arr = HelloApplication.exams.get(x).getExam_report().getHisto().getFrequency_arr();
            ArrayList<String> s=new ArrayList<String>();
            HelloApplication.exams.get(x).getExam_report().getHisto().display();
            for(int i = 0; i < arr.length; i++)
                series.getData().add(new XYChart.Data(new String(String.valueOf(i)),arr[i] ));

            toughquestions.setText(HelloApplication.exams.get(x).getExam_report().display_tough());
    ...
    public void initialize() {
        for(int i=0;i<HelloApplication.exams.size();i++)
            examcode.getItems().add(HelloApplication.exams.get(i).getID());
    ...
    @FXML
    public void ver(){
        toughquestions.setText(HelloApplication.exams.get(examcode.getSelectionModel().getSelectedIndex()).getExam_report().showrank());
    }
}